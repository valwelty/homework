...
```{r, tidy=TRUE}
# The largest coefficient of the model is for gender, where it equals 4.50875, but of course this depends partially on the scale of the variable and does not imply that it has the highest effect on 'los'
```
...
p_val <- 2*pt(est[2,3], df=7, lower.tail=FALSE)
p_val
```
...
```{r, tidy=TRUE}
t.test(women[,5], men[,5])
t.test(women[,5], men[,5], var.equal=TRUE)
t.test(women[,5], men[,5])$p.value
t.test(women[,5], men[,5], var.equal=TRUE)$p.value

mod <- lm(los ~ gender, dat=x)
summary(mod)

# The p-value from the t test using equal variances, 0.02243, matches the p-value for gender from the model summary (0.02243). The p-value from the t test not using equal variances, 0.02205, is not equal to these but not far off either.
```
